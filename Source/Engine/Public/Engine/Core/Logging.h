...
#define VOLUCRIS_STATIC_LOG(name, level) static volucris::Logger VOLUCRIS_LOGGER_NAME(name) = volucris::Logger(#name, volucris::Logger::Level::level);

#define V_LOG(name, level, format_s, ...) VOLUCRIS_LOGGER_NAME(name).log(volucris::Logger::Level::level, fmt::format(format_s, ##__VA_ARGS__).c_str());
...
#define V_LOG_WARN(name, format_s, ...) V_LOG(name, Warning, format_s, ##__VA_ARGS__)
